...
function getCurrentChapter() {
  for (let i = 0; i < chapters.length; i++) {
    if (
      currentLevelNum >= chapters[i].start &&
      currentLevelNum <= chapters[i].end
    ) {
      return i;
    }
  }
...
function moveToNextPillar() {
  ...
  console.log(
    "Moved - Current pillar at:",
    nextLeft + "px",
    "Bridge at:",
    nextLeft + nextWidth + "px",
  );
...
function updateQuality() {
  ...
  if (isInCollege()) {
    ...
    document.getElementById("instruction").innerText =
      `⚠️ ĐẠI HỌC: KHÓ GẤP ĐÔI! Nhấn giữ chuột cẩn thận!${deathInfo}`;
    document.getElementById("instruction").style.color = "red";
    document.getElementById("instruction").style.fontWeight = "bold";
  } else if (deathCount > 0) {
    document.getElementById("instruction").innerText =
      `Nhấn giữ chuột để tích lũy LƯỢNG (Chết: ${deathCount} lần - Tốc độ +${deathCount * 2})`;
    document.getElementById("instruction").style.color = "orange";
    document.getElementById("instruction").style.fontWeight = "bold";
  } else {
    document.getElementById("instruction").innerText =
      "Nhấn giữ chuột để tích lũy LƯỢNG (độ dài cầu)";
...
function updateLivesDisplay() {
  ...
  livesDisplay.innerText =
    heart.repeat(lives) + emptyHeart.repeat(maxLives - lives);
}
...
function nextTurn() {
  ...
  const currentWidth = parseInt(pillarCurrent.style.width) || 100;
  const nextPillarLeft =
    currentPillarLeft + currentWidth + levels[currentLevel].gap;
  ...
  console.log(
    "Next pillar INITIAL at:",
    nextPillarLeft + "px",
    "Current pillar at:",
    currentPillarLeft + "px",
    "Gap:",
    levels[currentLevel].gap,
  );
...
function handleRetry() {
  ...
  console.log(
    `DEBUG: currentLevelNum trước khi tính checkpoint: ${currentLevelNum}`,
  );
  ...
  // Hiển thị thông báo
  const difficultyMsg =
    deathCount === 1 ? "" : ` (Độ khó tăng: +${(deathCount - 1) * 2})`;
  console.log(
    `🔁 VỀ CHECKPOINT Lớp ${checkpointLevel} - Lần ${deathCount}${difficultyMsg}`,
  );
...
function finishQuiz() {
  ...
  if (correctAnswers >= config.required) {
    ...
    console.log(
      `❌ FAIL QUIZ Lớp ${checkpointLevel} → Quay về Lớp ${config.failTo}`,
    );
